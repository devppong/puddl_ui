...
const renderCard = (value, state) => {
	...
	if (value === "total") {
		...
		let data =
			context_tokens_data && context_tokens_data.data
				? context_tokens_data.data
				: [];
		return (
			<div className="max-w-lg">
				<Flex className="mt-4">
		...
		let data =
			generated_tokens_data && generated_tokens_data.data
				? generated_tokens_data.data
				: [];
		return (
			<div className="max-w-lg">
				<Flex className="mt-4">
...
const getTotalMetrics = (value, state) => {
	let { tokens_data, context_tokens_data, generated_tokens_data } = state;
	if (value === "total") {
		return tokens_data && tokens_data.total_tokens
			? tokens_data.total_tokens
			: 0;
	} else if (value === "context") {
		return context_tokens_data && context_tokens_data.total_tokens
			? context_tokens_data.total_tokens
			: 0;
	} else if (value === "generated") {
		return generated_tokens_data && generated_tokens_data.total_tokens
			? generated_tokens_data.total_tokens
			: 0;
	}
};
