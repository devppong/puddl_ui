...
import { updateDateRange, updateFilters, validateApiKey, updateApiKey } from "./actions";
...
function Analytics() {
  const initialState = {
    ...
    date_range: [new Date(new Date().setDate(new Date().getDate() - 30)), new Date()]

};

const [state, dispatch] = useReducer(reducer, initialState);
const [apiKeyStatus, setApiKeyStatus] = useState<"warning" | "error" | "">("error");
...
return (
    <main className="bg-slate-50 p-6 sm:p-10">
        <Flex style={{display:'flex',alignItems:'center'}}>
            ...
            {
                apiKeyStatus === "" ? (
                    <Badge className="max-w-sm " color="green" style={{margin:'10px'}}>
                        <div style={{display:'flex',alignItems:'center',padding:'2px'}}>
                            <span className="animate-ping bg-green-50 opacity-75" style={{width:'10px'}}><StatusOnlineIcon/></span>
                            <span className="ml-2">Active</span> 
                        </div>
                    </Badge>
                ):(
                    <Badge className="max-w-sm " color="red" style={{margin:'10px'}}>
                    <div style={{display:'flex',alignItems:'center',padding:'2px'}}>
                        <span className="animate-ping bg-red-50 opacity-75" style={{width:'10px'}}><StatusOfflineIcon/></span>
            ...
            />
            <MultiSelectBox className="max-w-sm space-y-6 gap-6" onValueChange={handleUpdateFilters} value={state.filters} style={{margin:'10px',width:'230px'}}>
            ...
            <Input.Password  status={apiKeyStatus} className="max-w-sm" placeholder="OpenAI key" onChange = {onChangekey} value={apiKey}/>
        </Flex>
